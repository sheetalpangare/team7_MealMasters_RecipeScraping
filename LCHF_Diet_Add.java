...
public class LCHF_Diet_Add extends BaseClass {
	private static final Logger logger = LoggerFactory.getLogger(LCHF_Diet_Add.class);
	...
	List<String> includeIngredients = Arrays.asList("fish", "prawn", "poultry", "egg", "onion", "garlic", "turmeric",
			"ginger", "butter", "ghee", "hard cheese", "paneer", "cottage cheese", "sour cream", "greek yogurt",
			"hung curd", "almond", "pistachio", "brazil nut", "walnut", "pine nut", "hazelnut", "macadamia nut",
			"pecan", "hemp seed", "sunflower seed", "sesame seed", "chia seed", "flax seed", "blueberry", "blackberry",
			"strawberry");
	...
	public void scrapeReceipeUrls() throws Exception {
		...
		if (driver.getCurrentUrl().equals("https://www.tarladalal.com/#google_vignette")) {
			...
			adx.click();
			logger.info("closed the ad before clicking the Recipes link");
		}
		recipeslink.click();
	...
	public void recipeDetails(WebDriver driver, String recipeURL) throws Exception {
		...
		for (WebElement ingredient : ingredientElements) {
			...
			for (String include : includeIngredients) {
				if (text.contains(include.toLowerCase())) {
					logger.info("add recipe (contains added ingredient: " + include);
					return;
				}
			}
		}
		...
		try {
			...
			logger.info("Preparation_method: " + method.getText());
		...
		try {
			...
			logger.info("Nutrients Values: " + Nutrients.getText());
		...
		logger.info("--------------------");
		DBConnection.createTable("lchf_diet_add");
		DBConnection.insertRecipe(recipe, "lchf_diet_add");
